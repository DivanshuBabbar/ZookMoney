...
class MerchantPaymentGatePayController extends Controller
{
    ...
    public function merchantlinkGateWay($lang, $merchant_id)
    {
      ...
      $data['user']=$user;
      $data['item_name'] = 'Merchant Payment';
      $callback_url = '/merchant_gatepay_callback';
      session()->put('gateway_callback_url',$callback_url);
      $total_amount = 0;
      if(session()->has('PurchaseRequestTotal'))
      {
        $total_amount = session()->get('PurchaseRequestTotal');
      }
      $data['total_amount'] = $total_amount;
      // $redirect_url = route('paymentLinkStoreFront',[app()->getLocale(),$payment_id]);
      // session()->put('redirect_url',$redirect_url);
      return view('gatepay.merchant_gatepay',$data);
    }
    public function merchant_gatepay_callback()
    {
        $merchant_id = session()->get('merchant_id');
        $merchant = Merchant::find($merchant_id);
        $success_link = isset($merchant->success_link) ? $merchant->success_link:'';
        $fail_link = isset($merchant->fail_link) ? $merchant->fail_link:'';
    ...
    private function postData($paymentResponseData)
    {
        ...
        Transaction::create([
            ...
            'money_flow'    => '+',
            'activity_title'    =>  'Merchant payment',
        ...
        if(session()->has('link_owner_id'))
        {
            session()->forget('link_owner_id');
        }
    }
    public function merchant_gatepay_process_payment()
    {
        ...
        if (isset($_POST) && count($_POST) > 0) 
        {
            ...
            if ($validation === true) {
                if($paymentOption == 'strowallet')
                {
                    ...
                    $merchant = Merchant::find($merchant_id);
                    $fail_link = isset($merchant->fail_link) ? $merchant->fail_link:'';
                    $paymentData = $strowalletService->proccesPayment($insertData,$fail_link);
