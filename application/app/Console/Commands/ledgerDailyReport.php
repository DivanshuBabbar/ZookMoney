...
class ledgerDailyReport extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ledgerDaily:report';
     ...
     * @var string
     */
    protected $description = 'send ledger daily report to merchant';
    ...
    {   
        try {
            ...
            if (!File::exists(storage_path()."/user_docs/".$date."/payIn")) {
                File::makeDirectory(storage_path() . "/user_docs/".$date."/payIn", 0777, true);
            }
            ...
            if ($chunks) {
                ...
                foreach ($chunks as $key => $user) {
                    foreach ($user as $key => $user_value) {
                        ...
                        $filename =  storage_path("user_docs/".$date."/payIn/".$user_value['name']."_ledgerDailyReport.csv");
                        ...
                        try {
                            ...
                            {
                                $message->to($email)->subject('LederDailyReport');
                        ...
                        $name = $user_value['name']."_".$date."_ledgerDailyReport.csv";
                        Storage::disk('payin_google')->put($name, File::get($filename));
                        unlink($filename);
                    }
                }
            }
            rmdir(storage_path("user_docs/".$date."/payIn/"));
