                ...
                <div class="menu">
                    <ul class="list">
                        ...
                        <li class="{{ (Route::is('home') ? 'active open' : '') }}"> 
                            <a href="{{ route('home', app()->getLocale()) }}"><i class=" icon-layers"></i><span>{{__('Dashboard')}}</span></a>
                        </li>
                        {{--
                        ...
                        <li class="{{ (Route::is('sendMoneyForm') ? 'active open' : '') }}"> 
                         ...
                         {{-- <li class="{{ (Route::is('vcard') ? 'active open' : '') }}"> 
                            <a href="{{ route('vcard',  app()->getLocale()) }}"><i class="icon-credit-card"></i><span>
                            {{__('Virtual Card')}}</span></a>
                        </li> --}}
                        {{-- <li class="{{ (Route::is('airtime') ? 'active open' : '') }}"> 
                            <a href="{{ route('airtime',  app()->getLocale()) }}"><i class="icon-speedometer"></i><span>
                            {{__('Top-Up Mobile')}}</span></a>
                        </li> --}}
                         {{-- <li class="{{ (Route::is('airtime') ? 'active open' : '') }}"> 
                            <a href="{{url('/')}}/{{app()->getLocale()}}/giftCards"><i class="icon-speedometer"></i><span>
                            {{__('Gift Cards')}}</span></a>
                        </li>  --}}
                        <li class="{{ (Route::is('paymentlinks') ? 'active open' : '') }}"> 
                            <a href="{{ route('paymentlinks',  app()->getLocale()) }}"><i class="icon-link"></i><span>
                            {{__('Payment link')}}</span></a>
                        </li>
                        {{--
                        <li class="{{ (Route::is('cards') ? 'active open' : '') }}"> 
                            <a href="{{ route('cards',  app()->getLocale()) }}"><i class="icon-credit-card"></i><span>
                            {{__('Cards')}}</span></a>
                        </li>
                         --}}
                        ...
                        <li class="{{ (Route::is('requestMoneyForm') ? 'active open' : '') }}"> 
                         ...
                         {{-- <li class="{{ (Route::is('escrow') ? 'active open' : '') }}"> 
                            <a href="{{route('escrow',  app()->getLocale())}}"><i class="icon-folder-alt"></i><span>
                            {{__('Escrow system')}}</span></a>
                        </li> --}}
                         {{-- <li class="{{ (Route::is('exchange_currency.list') ? 'active' : '') }}"> 
                            <a href="{{route('exchange_currency.list', app()->getLocale())}}"><i class="icon-arrow-right"></i>
                        ...
                        --}}
                         ...
                         {{-- <li>
                            <a href="javascript:void(0);" class="menu-toggle waves-effect waves-block"><i class="icon-bar-chart"></i><span>{{__('P2P Trade')}}</span></a>
                            <ul class="ml-menu" style="z-index:99999 !important;">
                                <li><a href="{{route('offerbook',  app()->getLocale())}}" class=" waves-effect waves-block">{{__('All Offers')}}</a></li>
                                <li><a href="{{route('mybook',  app()->getLocale())}}" class=" waves-effect waves-block">{{__('My Trades')}}</a></li>
                                 <li><a href="{{route('myclosed',  app()->getLocale())}}" class=" waves-effect waves-block">{{__('Completed Trades')}}</a></li>
                            </ul>
                        </li> --}}
                        <li class="{{ (Route::is('my_tickets') ? 'active open' : '') }}"> 
                            <a href="{{ route('my_tickets',  app()->getLocale()) }}"><i class="icon-call-out"></i><span>
                            {{__('Support Ticket')}}</span></a>
                        </li>
                        @if(!empty(auth()->user()->is_merchant))
                        <li class="{{ (Route::is('mymerchants') ? 'active open' : '') }}"> 
                            <a href="{{ route('mymerchants',  app()->getLocale()) }}"><i class="icon-docs"></i><span>
                            {{__('Api docs')}}</span></a>
                        </li>
                        @endif
                        <li class="{{ (Route::is('mydeposits') ? 'active open' : '') }}"> 
                            <a href="{{route('mydeposits',  app()->getLocale())}}" style="width: 51vh;"><i class="icon-arrow-down"></i><span>
                            {{__('Zook Wallet Ledger')}}</span></a>
                        </li>
                        <li class="{{ (Route::is('withdrawal.index') ? 'active open' : '') }}"> 
                         ...
                         @if(Auth::user()->payin_status == 1)
                         <li class="{{ (Route::is('ledger') ? 'active open' : '') }}"> 
                            <a href="{{ route('ledger', app()->getLocale()) }}"><i class="icon-folder-alt"></i><span>{{__('Payin Ledger')}}</span></a>
                        </li>
                        <li class=""> 
                            <a href="https://zookpe.com/application/storage/app/public/uploads/ZookPe%20Payin%20Integration%20Docs.pdf" target="_blank"><i class='icon-credit-card'></i><span>{{__('Payin Api')}}</span></a>
                        </li>
                        @endif
                        @if(!empty(auth()->user()->is_merchant))
                        <li class="{{ (Route::is('logs') ? 'active open' : '') }}"> 
                            <a href="{{ route('logs', app()->getLocale()) }}"><i class='icon-bar-chart'></i><span>{{__('Logs')}}</span></a>
                        </li>
                        @if(Auth::user()->payout_status == 1)
                         <li class="{{ (Route::is('payout') ? 'active open' : '') }}"> 
                            <a href="{{ route('payout', app()->getLocale()) }}"><i class='icon-speedometer'></i><span>{{__('Payout Ledger')}}</span></a>
                        </li>
                        <li class="{{ (Route::is('bulk_payout') ? 'active open' : '') }}"> 
                            <a href="{{ route('bulk_payout.index', app()->getLocale()) }}"><i class='icon-settings'></i><span>{{__('Bulk Payout')}}</span></a>
                        </li>
                        <li class=""> 
                            <a href="https://zookpe.com/application/storage/app/public/uploads/ZookPe%20Payout%20Integration%20Docs.pdf" target="_blank"><i class='icon-credit-card'></i><span>{{__('Payout Api')}}</span></a>
                        </li>
                        @endif
                        @endif
                        <li class="{{ (Route::is('developerlist') ? 'active open' : '') }}"> 
                            <a href="{{ route('developer_tools.index',app()->getLocale()) }}"><i class='icon-control-play'></i><span>{{__('Developer Tools')}}</span></a>
                        </li>
                        <li class="{{ (Route::is('whitelistaccount') ? 'active open' : '') }}"> 
                            <a href="{{ route('whitelistaccount.index',app()->getLocale()) }}"><i class='icon-grid'></i><span>{{__('Whitelist Account')}}</span></a>
                        </li>
                        @if(Auth::user()->role_id != 1)
                        <li class="{{ (Route::is('my_vouchers') ? 'active open' : '') }}"> 
                            <a href="{{url( app()->getLocale().'/')}}/my_vouchers"><i class="icon-present"></i><span>
                            {{__('Vouchers')}}</span></a>
                        </li>
                        @endif
