                        ...
                        <tbody>
                            @foreach(session()->get('PurchaseRequest')->data->items as $item)
                                <tr>
                                    <td>{{$loop->index}}</td>
                                    <td>{{$item->qty}}</td>
                                    <td>{{session()->get('PurchaseRequest')->currency->symbol}}{{\App\Helpers\Money::instance()->value($item->price)}} </td>
                                    <td>{{session()->get('PurchaseRequest')->currency->symbol}}{{\App\Helpers\Money::instance()->value($item->price * $item->qty)}}</td>
                                    <td>{{$item->name}}</td>
                                </tr>
                            @endforeach
                        </tbody>
