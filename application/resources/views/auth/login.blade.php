            ...
            <div class="login-card">
                ...
                <form class="form" method="POST" action="{{ route('login', app()->getLocale()) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input  id="email" type="email" class="form-control input-field {{ $errors->has('email') ? 'is-invalid' : '' }}" name="email" value="{{ old('email') }}" placeholder="{{ __('Email') }}" required autofocus>
                    ...
                    <button type="submit" class="btn btn-primary" disabled>@lang('Login') </button>
                    {{--<div>
                        <a href="{{ route('password.request', app()->getLocale()) }}" class="forgot-password float-right">Lost Password?</a>
                    </div>--}}
                </form>
            </div>
            <div class="join-now-outer text-center">
                <a class="mb-0" href="{{url('/')}}/{{app()->getLocale()}}/register" >Join now, create your FREE account</a>
            </div>
        </div>
    </div>
